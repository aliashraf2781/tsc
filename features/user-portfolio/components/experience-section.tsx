  ...
  return (
    ...
    >
      {experiences.length > 0 ? (
          ...
          {experiences.map((exp, idx) => (
            ...
            >
              ...
              <div className="pe-12 ps-2">
                ...
                <p className="text-[13px] text-[#525252] font-semibold line-clamp-1">{exp.department}</p>
                <p className="text-[11px] text-[#6B7280] mt-1 font-semibold">
                  <span dir="ltr">{exp.startDate}</span> -{" "}
                  {exp.currentlyWorking ? t("experience.present") : <span dir="ltr">{exp.endDate || ""}</span>}
                </p>
              </div>
              ...
              <div className="mt-3 pt-2 border-t border-gray-100 flex items-center gap-1.5 text-xs text-[#006EA8] truncate">
                ...
                {exp.attachment ? (
                  ...
                  <span className="truncate text-gray-400 text-[11px] max-w-[130px]">
                    {exp.attachmentFile ? exp.attachmentFile.name : t("experience.noAttachment")}
                  </span>
                )}
              </div>
            </div>
        ...
        <p className="text-sm text-gray-400 italic">{t("experience.empty")}</p>
      )}
    </PortfolioSectionCard>
  )
}
