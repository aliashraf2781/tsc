...
import { deleteCountryAction } from "@/features/admin/actions/admin-actions"
import { Globe, Plus } from "lucide-react"
import { AdminPageLayout } from "./admin-page-layout"
import { CountryCard } from "./country-card"
import { EDIT_LOCALES, type AdminCountry, type LocaleKey } from "@/features/admin/lib/country-form-schema"
import { makeCountryKey } from "@/features/admin/lib/country-form-utils"
...
function mapCountriesToEntries(countries: AdminCountry[]): CountryEntry[] {
  return countries.map((c) => ({ key: String(c.id), id: c.id, country: c }))
}

export function AdminCountriesPanel({
  countries,
  locale,
}: {
  ...
  const [entries, setEntries] = useState<CountryEntry[]>(() => mapCountriesToEntries(countries))
  ...
  useEffect(() => {
    setEntries((prev) => {
      ...
      const serverEntries = mapCountriesToEntries(countries)
      const draftToKeep =
  ...
  }, [countries, newEntryKey])

  function addCountry() {
    const key = makeCountryKey()
    setNewEntryKey(key)
    setEntries((prev) => [...prev, { key }])
  }

  function handleDeleteRequest(entry: CountryEntry) {
    if (!entry.id) {
  ...
  async function confirmDelete() {
    ...
    try {
      const result = await deleteCountryAction(deleteTarget.id, locale)
      if (!result.ok) {
  ...
  return (
    <AdminPageLayout
      title={t("manageCountries")}
      description={t("manageCountriesDesc")}
      action={
        <PrimaryButton
          type="button"
          onClick={addCountry}
      ...
      <div className="space-y-6">
        ...
        {entries.length === 0 && (
          <div className="rounded-[12px] border border-dashed border-[#78A3BE] bg-[#F8FBFF] py-16 text-center">
            <Globe className="mx-auto h-10 w-10 text-[#78A3BE]" />
            <p className="mt-3 text-sm text-[#9CA3AF]">{t("noCountries")}</p>
          </div>
        )}
